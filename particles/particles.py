...
########### beam optics implemented using dE (as in PyOrbit) ##########
    ...
    def epsn_x_dE(self):
        dE = self._dE()
        return emittance(self.x, self.xp,dE)*self.betagamma

    def epsn_y_dE(self):
        dE = self._dE()
        return emittance(self.y, self.yp, dE)*self.betagamma

    def epsn_z_dE(self):
        return self.epsn_z()

    def alpha_Twiss_x_dE(self):
        dE = self._dE()
        return getAlpha(self.x, self.xp, dE)

    def alpha_Twiss_y_dE(self):
        dE = self._dE()
        return getAlpha(self.y, self.yp, dE)

    def beta_Twiss_x_dE(self):
        dE = self._dE()
        return getBeta(self.x, self.xp, dE)

    def beta_Twiss_y_dE(self):
        dE = self._dE()
        return getBeta(self.y, self.yp, dE)

    def gamma_Twiss_x_dE(self):
        dE = self._dE()
        return getGamma(self.x, self.xp, dE)

    def gamma_Twiss_y_dE(self):
        dE = self._dE()
        return getGamma(self.y, self.yp, dE)

    def dispersion_x_dE(self):
        dE = self._dE()
        return dispersion(self.x, dE, self.beta, self.p0)

    def dispersion_y_dE(self):
        dE = self._dE()
        return dispersion(self.y, dE, self.beta, self.p0)





def dispersion(u, dE, beta, momentum):
    covariance = cp.cov
...
def emittance(u, up, dE):
    cov_u2 = abs(cp.cov(u, u))
    cov_up2 = abs(cp.cov(up, up))
    cov_u_up = cp.cov(u, up)
...
def getAlpha(u, up, dE):
    ...
    cov_up_dE = cp.cov(up, dE)
    cov_dE2 = abs(cp.cov(dE, dE))
    alpha = -(cov_u_up - cov_u_dE*cov_up_dE/cov_dE2)/emittance(u,up,dE)
    return alpha
...
def getGamma(u, up, dE):
    cov_up2 = abs(cp.cov(up,up))
    cov_up_dE = cp.cov(up,dE)
    cov_dE2 = abs(cp.cov(dE, dE))
    gamma = (cov_up2 - cov_up_dE*cov_up_dE/cov_dE2)/emittance(u, up, dE)
    return gamma
