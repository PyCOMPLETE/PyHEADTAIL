...
class Slicer(object):
    ...
    def _set_longitudinal_cuts(self, bunch):
        ...
        if self.nsigmaz == None:
            z_cut_tail = np.min(bunch.z[0:bunch.n_macroparticles - bunch.n_macroparticles_lost])
            z_cut_head = np.max(bunch.z[0:bunch.n_macroparticles - bunch.n_macroparticles_lost])
        else:
            mean_z  = cp.mean(bunch.z[0:bunch.n_macroparticles - bunch.n_macroparticles_lost])
            sigma_z = cp.std(bunch.z[0:bunch.n_macroparticles - bunch.n_macroparticles_lost])
    ...
    def epsn_x(self, bunch):
        return self._epsn(bunch.x, bunch.xp) * bunch.gamma * bunch.beta * 1e6

    def epsn_y(self, bunch):
        return self._epsn(bunch.y, bunch.yp) * bunch.gamma * bunch.beta * 1e6
