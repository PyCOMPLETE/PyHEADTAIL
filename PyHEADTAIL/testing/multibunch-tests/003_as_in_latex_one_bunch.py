...
i_z_source = np.argmin(np.abs(z_centers - (-i_bucket_source * bucket_length)))
i_z_a = i_z_source - n_slices//2
n_source = n_slices + 10

i_z_target = np.argmin(np.abs(z_centers - (-i_bucket_target * bucket_length)))
i_z_c = i_z_target - n_slices//2
n_target = n_slices + 20

z_source = z_centers[i_z_a : i_z_a+n_source]
z_target = z_centers[i_z_c : i_z_c+n_target]

dip_moment_slice_source = dip_moment_slice[i_z_a : i_z_a+n_source]
...
z_wake_abcd = np.arange(z_c - z_b, z_d - z_a, dz)
W_r_abcd = (
      R_s * omega_r**2 / (Q * omega_bar) * np.exp(alpha_t * z_wake_abcd / c)
      * np.sin(omega_bar * z_wake_abcd / c))# Wake definition
W_r_abcd[z_wake_abcd > 0] = 0
W_scaled_abcd = -e**2 / (p0_SI * c) * W_r_abcd # Put all constants in front of the wake

len_fft = len(W_scaled_abcd)

rho_hat = fft(dip_moment_slice_source, n=len_fft)
G_hat = fft(W_scaled_abcd)

phase_term = np.exp(-1j * 2 * np.pi * np.arange(0, len_fft) * (z_a - z_b) / (len_fft * dz))
dxp_fft_abcd = ifft(phase_term*rho_hat * G_hat).real

dxp_target = dxp_fft_abcd[0:n_target]
