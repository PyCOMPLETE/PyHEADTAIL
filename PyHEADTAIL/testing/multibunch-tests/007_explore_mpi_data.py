...
required_variables = ['age', 'beta', 't_centers',
                                    'n_macroparticles_per_slice',
                                    'mean_x', 'mean_y']
mpi_gatherer = mpi_data.MpiGatherer(slicer, required_variables)
mpi_gatherer.gather(allbunches)
mpi_gatherer.bunch_by_bunch_data
...
for i in range(n_turns):
    ...
    if i % 10 == 0:
        print('{:4d}   {:+.2e}   {:+.2e}   {:+.2e}   {:.2e}   {:.2e}   {:.2e}      {:.2f}      {:.2f}   {:2s}'.format(i, bunch.mean_x(), bunch.mean_y(), bunch.mean_z(), bunch.epsn_x(), bunch.epsn_y(), bunch.epsn_z(), bunch.sigma_z(), bunch.sigma_dp(), str(time.time() - t0)))
