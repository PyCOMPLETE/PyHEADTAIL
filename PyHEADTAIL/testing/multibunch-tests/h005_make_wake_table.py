...
n_turns_wt = 20
z_wt = np.arange((n_wake)*0.7*dz+machine.circumference, (-n_wake*dz - n_turns_wt*machine.circumference), -dz/10)[::-1]

W_r_wt = (R_s * omega_r**2 / (Q * omega_bar) * np.exp(alpha_t * z_wt / c) * np.sin(omega_bar * z_wt / c))
...
fig100 = plt.figure(100, figsize=(6.4*1.5, 4.8))
ax100 = fig100.add_subplot(121)
ax101 = fig100.add_subplot(122)

ax100.plot(z_wt, W_scaled_wt)
ax100.plot(z_wake_mt.T, Wmt_scaled.T, label='Wake')
ax100.set_xlabel('z [m]')
ax100.set_ylabel('W(z)')

ax101.plot(t_wt, w_wt)
ax101.set_xlabel('t [ns]')
ax101.set_ylabel('W(t) / ($e^2/E_0$)')
