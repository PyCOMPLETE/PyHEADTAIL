...
class Monitor(object):
    """
    Abstract base class for monitors. A monitor can request statistics
    data such as mean value and standard deviation and store the results
    in an HDF5 file.
    """
    ...
    def dump(bunch):
        """
        Write particle data given by bunch (instance of Particles class)
        to buffer and/or file at the specific time the method is called.
        Data can e.g. be bunch-specific, slice_set-specific or
        particle-specific.
        """
        pass
...
class BunchMonitor(Monitor):
    ...
    def dump(self, bunch):
        """
        Evaluate the statistics like mean and standard deviation for the
        given bunch and write the data to the HDF5 file. Make use of a
        buffer to reduce the number of writing operations to file. This
        helps to avoid IO errors and loss of data when writing data
    ...
    def _create_file_structure(self, parameters_dict):
        ...
        datasets). One group is created for bunch-specific data. One
        dataset for each of the quantities defined in
        ...
        try:
            ...
            print('Creation of bunch monitor file %s failed.' % self.filename)
            raise

    def _write_data_to_buffer(self, bunch):
        """
        Store the data in the self.buffer dictionary before writing them
        to file. The buffer is implemented as a shift register.
        """
        for stats in self.stats_to_store:
    ...
    def _write_buffer_to_file(self):
        ...
        step and repeat it again after self.write_buffer_every. As long
        as self.buffer_size is not exceeded, no data are lost.
        '''
        try:
...
class SliceMonitor(Monitor):
    """
    Class to store bunch- and slice_set-specific data to a HDF5 file.
    This monitor uses two buffers (shift registers) to reduce the number
    of writing operations to file. This also helps to avoid IO errors
    and loss of data when writing to a file that may become temporarily
    unavailable (e.g. if file is located on network) during the
    simulation.
    """
    ...
    def dump(self, bunch):
        """
        Evaluate the statistics like mean and standard deviation for the
        given slice_set and the bunch and write the data to the buffers
        and/or to the HDF5 file. The buffers are used to reduce the
        number of writing operations to file. This helps to avoid IO
    ...
    def _create_file_structure(self, parameters_dict):
        """
        Initialize HDF5 file and create its basic structure (groups and
        datasets). Two groups are created, one for slice_set-specific
        and one for bunch-specific data. One dataset for each of the
        quantities defined in self.stats_to_store is generated.
        If specified by the user, write the contents of the
        parameters_dict as metadata (attributes) to the file.
        Maximum file compression is activated.
        """
        try:
            ...
            print('Creation of slice monitor file %s failed.' % self.filename)
            raise
    ...
    def _write_data_to_buffer(self, bunch):
        ...
        Store the data in the self.buffer dictionary before writing them
        to file. The buffer is implemented as a shift register.
        To find the slice_set-specific data, a slice_set, defined by the
        ...
        for stats in self.stats_to_store:
            ...
            Handle the different statistics quantities, which can
            either be methods (like mean(), ...) or simply attributes
            (n_macroparticles) of the bunch or slice_set.
            '''
            # Bunch-specific data.
            try:
...
class ParticleMonitor(Monitor):
    ...
    def _create_file_structure(self, parameters_dict):
        ...
        try:
            ...
            print('Creation of particle monitor file %s failed.'
                  % self.filename)
            raise
