...
class WakeKick(object):
    ...
    def __init__(self, wake_function, slicer_mode):
        """
        Universal constructor for WakeKick objects. The slicer_mode is
        passed only to decide about which of the two implementations of
        the convolution the self._convolution method is bound to.
        """
        ...
        if slicer_mode == 'uniform_charge':
            ...
            raise ValueError('Unknown slicer_mode. Must be either ' +
                             'uniform_bin or uniform_charge.')

    @abstractmethod
    def apply(self, bunch, slice_set):
        """
        Calculates and applies the corresponding wake kick to the bunch
        conjugate momenta using the given slice_set. Only particles
        within the slicing region, i.e particles_within_cuts (defined
        by the slice_set) experience a kick.
        """
        pass
    ...
    def _convolution_dot_product(self, bunch, slice_set, beam_profile):
        """
        Implementation of the convolution of wake_field and beam_profile
        using the numpy dot product. To be used with the
    ...
    def _convolution_numpy(self, bunch, slice_set, beam_profile):
        """
        Implementation of the convolution of wake_field and beam_profile
        using the numpy built-in numpy.convolve method. Recommended use
        with the 'uniform_bin' slicer mode for higher performance.
        """
...
class ConstantWakeKickX(WakeKick):
    ...
    def apply(self, bunch, slice_set):
        zeroth_moment = slice_set.n_macroparticles
...
class DipoleWakeKickX(WakeKick):
    ...
    def apply(self, bunch, slice_set):
        first_moment_x = slice_set.n_macroparticles * slice_set.mean_x(bunch)
...
class DipoleWakeKickXY(WakeKick):
    ...
    def apply(self, bunch, slice_set):
        first_moment_y = slice_set.n_macroparticles * slice_set.mean_y(bunch)
