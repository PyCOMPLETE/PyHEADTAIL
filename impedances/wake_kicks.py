...
class WakeKick(object):
    ...
    def _wake_factor(self, bunch):
        ...
        wake_factor = -(bunch.charge) ** 2 / (bunch.mass * bunch.gamma * (bunch.beta * c) ** 2) * bunch.particlenumber_per_mp

        return wake_factor


    def _convolution_dot_product(self, bunch, slices, beam_profile):
    ...
    def _convolution_numpy(self, bunch, slices, beam_profile):
...
class ConstantWakeKickX(WakeKick):
    ...
    def apply(self, bunch, slices):
        ...
        constant_kick = self._wake_factor(bunch) * self._convolution(bunch, slices, zeroth_moment)
...
class DipoleWakeKickX(WakeKick):
    ...
    def apply(self, bunch, slices):
        ...
        dipole_kick_x = self._wake_factor(bunch) * self._convolution(bunch, slices, first_moment_x)
...
class DipoleWakeKickXY(WakeKick):
    ...
    def apply(self, bunch, slices):
        ...
        dipole_kick_xy = self._wake_factor(bunch) * self._convolution(bunch, slices, first_moment_y)
...
class DipoleWakeKickY(WakeKick):
    ...
    def apply(self, bunch, slices):
        ...
        first_moment_y = slices.n_macroparticles * slices.mean_y(bunch)
        dipole_kick_y = self._wake_factor(bunch) * self._convolution(bunch, slices, first_moment_y)
...
class DipoleWakeKickYX(WakeKick):
    ...
    def apply(self, bunch, slices):
        ...
        first_moment_x = slices.n_macroparticles * slices.mean_x(bunch)
        dipole_kick_yx = self._wake_factor(bunch) * self._convolution(bunch, slices, first_moment_x)
...
class QuadrupoleWakeKickX(WakeKick):
    ...
    def apply(self, bunch, slices):
        ...
        zeroth_moment = slices.n_macroparticles
        quadrupole_kick_x = self._wake_factor(bunch) * self._convolution(bunch, slices, zeroth_moment)
...
class QuadrupoleWakeKickXY(WakeKick):
    ...
    def apply(self, bunch, slices):
        ...
        zeroth_moment = slices.n_macroparticles
        quadrupole_kick_xy = self._wake_factor(bunch) * self._convolution(bunch, slices, zeroth_moment)
...
class QuadrupoleWakeKickY(WakeKick):
    ...
    def apply(self, bunch, slices):
        ...
        zeroth_moment = slices.n_macroparticles
        quadrupole_kick_y = self._wake_factor(bunch) * self._convolution(bunch, slices, zeroth_moment)
...
class QuadrupoleWakeKickYX(WakeKick):
    ...
    def apply(self, bunch, slices):
        ...
        zeroth_moment = slices.n_macroparticles
        quadrupole_kick_yx = self._wake_factor(bunch) * self._convolution(bunch, slices, zeroth_moment)
