...
plt.subplot(2,3,1)
plt.plot(beam_x)
plt.plot(2*np.exp(p1_x + tt * p0_x), label= 'Fitting curve \nEvaluated Damping time:%.2e [s],\nExpected Damping time:%.2e [s]'%((-1/p0_x)*(machine.circumference/c),synchdamp_x*(machine.circumference/c)), lw=2, color = 'red')
plt.legend (loc=0, fontsize = 10)
plt.ylabel('x [m]');plt.xlabel('Turn')
plt.gca().ticklabel_format(style='sci', scilimits=(0,0),axis='y')
plt.subplot(2,3,2)
plt.plot(beam_y)
plt.plot(2*np.exp(p1_y + tt * p0_y), label= 'Fitting curve \nEvaluated Damping time:%.2e [s],\nExpected Damping time:%.2e [s]'%((-1/p0_y)*(machine.circumference/c),synchdamp_y*(machine.circumference/c)), lw=2, color = 'red')
plt.legend (loc=0, fontsize = 10)
plt.ylabel('y [m]');plt.xlabel('Turn')
plt.gca().ticklabel_format(style='sci', scilimits=(0,0),axis='y')
plt.subplot(2,3,3)
plt.plot(beam_z)
plt.plot(2*np.exp(p1_z + tt * p0_z)+ beam_z[-1], label= 'Fitting curve \nEvaluated Damping time:%.2e [s],\nExpected Damping time:%.2e [s]'%((-1/p0_z)*(machine.circumference/c),synchdamp_z*(machine.circumference/c)), lw=2, color = 'red')
plt.legend (loc=0, fontsize = 10)
plt.ylabel('z [m]');plt.xlabel('Turn')
plt.gca().ticklabel_format(style='sci', scilimits=(0,0),axis='y')
plt.subplot(2,3,4)
plt.plot(epsx)
plt.axhline(epsx[-1], label= 'Fitting curve \nEvaluated Equilibrium emittance:%.2e [s],\nExpected Equilibrium emittance:%.2e [s]'%(epsx[-1],eq_emit_x), lw=2, color = 'red')
plt.legend (loc=0, fontsize = 10)
plt.ticklabel_format(useOffset=False, style='sci', scilimits=(0,0),axis='y')
plt.ylabel('$\epsilon_x$ [m.rad]');plt.xlabel('Turn')
plt.subplot(2,3,5)
plt.plot(epsy)
plt.axhline(epsy[-1], label= 'Fitting curve \nEvaluated Equilibrium emittance:%.2e [s],\nExpected Equilibrium emittance:%.2e [s]'%(epsy[-1],eq_emit_y), lw=2, color = 'red')
plt.legend (loc=0, fontsize = 10)
plt.ticklabel_format(useOffset=False, style='sci', scilimits=(0,0),axis='y')
plt.ylabel('$\epsilon_y$ [m.rad]');plt.xlabel('Turn')
plt.subplot(2,3,6)
plt.plot(sdp)
plt.axhline(sdp[-1], label= 'Fitting curve \nEvaluated Equilibrium momentum spread:%.2e [s],\nExpected Equilibrium momentum spread:%.2e [s]'%(sdp[-1],eq_sig_dp), lw=2, color = 'red')
plt.legend (loc=0, fontsize = 10)
plt.ticklabel_format(useOffset=False, style='sci', scilimits=(0,0),axis='y')
