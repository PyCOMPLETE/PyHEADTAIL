...
class MpiGatherer(object):
    """
        MpiGather is an object, which gathers slice data from other processors. The data between processors are shared
        by using MPI message-passing system. The data can be accessed by calling the total_data or bunch_by_bunch_data
        objects. The object total_data includes slice set data from all bunches in all processors in one array, whereas
        the object bunch_by_bunch_data is a list of objects, which includes slice set data from individual bunches.
        Those objects emulates orginal slice_set objects, data can be accessed by calling same variable names
        (e.g. mean_x, sigma_x and n_macroparticles_per_slice) than in the slice set object. For example, by calling
        total_data.mean_x returns a single array, which includes mean_x values from all slices in all bunches and
        bunch_by_bunch_data[0].mean_x returns  mean_x values from the slice set of the first bunch
    ...
    def __init__(self, slicer, required_variables):
        ...
        :param required_variables: a list of variable names in a slice set object which are shared
        (e.g. n_macro_particles, mean_x, sigma_y, etc)
        """
        ...
        self._n_bunches = None  # total number of bunches simulated in all processors
        self._n_local_bunches = None # number of bunches simulated in this rank
        self._local_bunch_indexes = None # list indexes for bunches simulated in this rank
        ...
        # the names of the variables in the slice set, which are shared between processors
    ...
    def gather(self,superbunch):
        ...
        if self._slicer.config[3] is not None:
            ...
            for i, b in enumerate(self.bunch_list):
                z_delay = b.mean_z()
                b.z -= z_delay
                s = b.get_slices(self._slicer, statistics=self._statistical_variables)
                b.z += z_delay
                s.z_bins += z_delay
        ...
        # another way to implement this could be to use separated Allgatherv(...) calls for each statistical
        # variable. It would allow to use memoryviews also in TotalDataSet object, but multiple Allgatherv(...)
        # calls might cost too much in time.
        self._mpi_comm.Allgatherv(
            self._output_buffer,
            [self._raw_data, self._slice_data_sizes, self._slice_data_offsets, MPI.DOUBLE])
    ...
    def _initialize_buffers(self,superbunch,slice_set_list):
        ###################################################################
        ##### Bunch distribution in the processors ########################
        ###################################################################
        ...
        bunches_temp_list = [self._bunch_distribution, bunch_distribution_sizes, bunch_distribution_offsets, MPI.INT32_T]

        self._mpi_comm.Allgatherv(
            bunches_in_this_rank,
            bunches_temp_list)
        ...
        raw_data_len = int(self._n_variables * self._n_slices * self._n_bunches)

        self._raw_data = np.zeros(raw_data_len)

        # FIXME: a problem in variable types because int(...) is required
        output_buffer_len = int(self._n_variables * self._n_slices * self._n_local_bunches)
        self._output_buffer = np.zeros(output_buffer_len)
        print output_buffer_len

        # FIXME: a problem in variable types because int(...) is required
        values_per_bunch = int(self._n_variables * self._n_slices)
        print values_per_bunch


        self._slice_data_sizes = np.ones(self._mpi_size) * self._bunch_distribution * values_per_bunch
        ...
        bin_data_sizes = np.ones(self._mpi_size) * self._bunch_distribution * bin_data_length
        ...
        self.total_data = TotalDataAccess(self._local_bunch_indexes,self._raw_data,self._raw_bin_data,self._required_variables,self._n_slices,self._n_bunches)

        for idx in xrange(self._n_bunches):
            self.bunch_by_bunch_data.append(BunchDataAccess(idx,self._raw_data,self._raw_bin_data,self._required_variables,self._n_slices))
    ...
    def _fill_output_buffer(self,superbunch,slice_set_list):
        ...
        for set_idx, slice_set in enumerate(slice_set_list):
            for idx, variable in enumerate(self._required_variables):
                idx_from = set_idx*self._n_slices*self._n_variables + idx*self._n_slices
                idx_to = set_idx*self._n_slices*self._n_variables + (idx+1)*self._n_slices
...
class BunchDataAccess(object):
    """
        An object, which emulates a slice set object by creating pointers to the shared data of the specific bunch.
        In other words, the data can be accessed by using same methods than in the case of slice set object (e.g. by
        calling SliceDataSetReference.mean_x or SliceDataSetReference.n_macroparticles_per_slice)
    """
    def __init__(self,bunch_idx,raw_data,raw_bin_data,variables,n_slices):
        """
        ...
        :param variables: a list of variable names in the slice set, which are shared between bunches
        ...
        for idx, variable in enumerate(self._variables):
            idx_from = self._bunch_idx*self._n_slices*self._n_variables+idx*self._n_slices
            idx_to = self._bunch_idx*self._n_slices*self._n_variables+(idx+1)*self._n_slices

            exec 'self.'+variable +' = np.array(self._v[idx_from:idx_to],copy = False)'

class TotalDataAccess(object):
    """
        Produces an object, which contains data from all bunches in a single list. For example, by calling
        TotalDataSet.mean_x returns a single list, which contains mean_x values of all slices in all bunches
    """

    def __init__(self,local_bunches,raw_data,raw_bin_data,variables,n_slices,n_bunches):
        """
        ...
        for i in xrange(n_bunches):
            if i in local_bunches:
                self.local_data_locations.append((i * self._n_slices,(i + 1) * self._n_slices))
        ...
        # self.local_data_position = (self._bunch_idx*self._n_slices,(self._bunch_idx+1)*self._n_slices)
    ...
    def update(self):
        # This method might be too slow (it copies data). It can be optimized after the decision about the data transfer
        # API between processors
        ...
        for idx, variable in enumerate(self._variables):
            ...
            for bunch_idx in xrange(self._n_bunches):
                ...
                buffer_from = bunch_idx*self._n_slices*self._n_variables+idx*self._n_slices
                buffer_to = bunch_idx*self._n_slices*self._n_variables+(idx+1)*self._n_slices
