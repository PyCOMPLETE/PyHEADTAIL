...
def relocate_lost_particles(beam, int[::1] alive):
    ...
    for i in xrange(last_alive-1, -1, -1):
        if not alive[i]:
            ...
            id[last_alive], bucket_id[last_alive] = t_id, t_bucket_id
