...
class TransverseSegmentMap(object):
    ...
    def __init__(self, s0, s1, alpha_x, beta_x, D_x, dQ_x, alpha_y, beta_y, D_y, dQ_y, *detuner_elements):
        ...
        # Calculate coefficients of TransverseSegmentMap.
        self._build_map()
    ...
    def _build_map(self):
        ...
        # Sine component.
        I[0, 0] = np.sqrt(self.beta_x[self.s1] / self.beta_x[self.s0])
        I[0, 1] = 0
        I[1, 0] = np.sqrt(1 / (self.beta_x[self.s0] * self.beta_x[self.s1])) * (self.alpha_x[self.s0] - self.alpha_x[self.s1])
        I[1, 1] = np.sqrt(self.beta_x[self.s0] / self.beta_x[self.s1])
        I[2, 2] = np.sqrt(self.beta_y[self.s1] / self.beta_y[self.s0])
        I[2, 3] = 0
        I[3, 2] = np.sqrt(1 / (self.beta_y[self.s0] * self.beta_y[self.s1])) * (self.alpha_y[self.s0] - self.alpha_y[self.s1])
        I[3, 3] = np.sqrt(self.beta_y[self.s0] / self.beta_y[self.s1])

        # Cosine component.
        J[0, 0] = np.sqrt(self.beta_x[self.s1] / self.beta_x[self.s0]) * self.alpha_x[self.s0]
        J[0, 1] = np.sqrt(self.beta_x[self.s0] * self.beta_x[self.s1])
        J[1, 0] = -np.sqrt(1 / (self.beta_x[self.s0] * self.beta_x[self.s1])) * (1 + self.alpha_x[self.s0] * self.alpha_x[self.s1])
        J[1, 1] = -np.sqrt(self.beta_x[self.s0] / self.beta_x[self.s1]) * self.alpha_x[self.s1]
        J[2, 2] = np.sqrt(self.beta_y[self.s1] / self.beta_y[self.s0]) * self.alpha_y[self.s0]
        J[2, 3] = np.sqrt(self.beta_y[self.s0] * self.beta_y[self.s1])
        J[3, 2] = -np.sqrt(1 / (self.beta_y[self.s0] * self.beta_y[self.s1])) * (1 + self.alpha_y[self.s0] * self.alpha_y[self.s1])
        J[3, 3] = -np.sqrt(self.beta_y[self.s0] / self.beta_y[self.s1]) * self.alpha_y[self.s1]
    ...
    def track(self, beam):
        ...
        # Phase advance (w/o. factor 2 np.pi, see below).
        dphi_x = self.dQ_x
        dphi_y = self.dQ_y
        ...
        for detuner in self.detuner_elements:
            ...
            dphi_x = dphi_x + detune_x
            dphi_y = dphi_y + detune_y
